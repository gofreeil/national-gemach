...
<svelte:head>
    <title>{ad.title} | הגמ"ח הארצי</title>
    {#if lp.pitch}
...
<div class="ad-landing" dir="rtl">
    ...
    <header class="al-hero" style:background={ad.gradient || 'linear-gradient(135deg, #f59e0b, #ea580c)'}>
        <div class="al-hero-inner" class:has-media={!!heroImage}>
            <div class="al-copy">
                {#if ad.logo}
                    <img src={ad.logo} alt="לוגו {ad.title}" width="88" height="88" decoding="async" class="al-logo" class:is-circle={logoCircle} />
                {/if}
            ...
            {#if heroImage}
                <div class="al-media">
                    <!-- תמונת ה-LCP של דף הנחיתה — עדיפות טעינה גבוהה, בלי lazy -->
...
<style>
    ...
    /* המידות של תמונת המפרסם אינן ידועות מראש (נשלפת מה-API), ולכן
       בנייד שמור לה מראש הגובה המרבי — הטקסט שמתחתיה לא קופץ כשהיא נטענת */
    .al-media {
        min-width: 0;
        min-height: 17rem;
        display: flex;
