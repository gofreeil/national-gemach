...
export interface Gemach {
    ...
    ownerId?: string;
    /** מצב פרסום (status1 ב-Strapi): 'active' מוצג באתר, 'draft' מוסתר.
     *  קיים רק בפריטים מנוהלים. */
    status?: string;
    /** חותמת "מאושר" — הגמ"ח עבר בדיקת מערכת (extra_fields.verified).
     *  ניתנת/מוסרת ע"י אדמין בתפריט הניהול. */
    verified?: boolean;
    /** גמ"ח חדש מהטופס הציבורי שממתין לעין אדמין (extra_fields.needs_review).
     *  מדליק את בועת ההתראות ותג "חדש" ב-/admin/gemachim עד שאדמין מסמן
     *  "נבדק" או מעניק תו תקן. */
    needsReview?: boolean;
    /** טיוטת-אורח (extra_fields.guest_claim) — גמ"ח שאורח שלח וממתין לפרסום
     *  או דחייה ידניים. נספר בבועת ההתראות; מובחן מטיוטות הגילוי החכם. */
    guestClaim?: boolean;
    /** מתי הרשומה נוצרה ב-Strapi (ISO). קיים רק בפריטים מנוהלים —
     *  לרשימה הסטטית אין תאריך. משמש את גרף הגידול בדף הסטטיסטיקה. */
    createdAt?: string;
    /** מתי הגמ"ח הצטרף למאגר בפועל (ISO): תאריך האישור/הפרסום של טיוטה, ואם לא
     *  עבר דרך טיוטה — תאריך היצירה. לפי זה נקבעים "גמ"חים חדשים" והמיון ברשימות. */
    joinedAt?: string;
}
